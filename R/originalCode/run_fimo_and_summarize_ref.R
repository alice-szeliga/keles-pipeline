...
for (k in 1:length(pwmNames)){
    ...
    fimo.command <- paste("/p/keles/MEME/meme_4.9.0/src/fimo --verbosity 1 --thresh 0.001 -oc ", 
...
out.1 <- matrix(1, length(d1), length(TF.names)) #minPval
colnames(out.1) <- TF.names
rownames(out.1) <- names(d1)

out.2 <- matrix(0, length(d1), length(TF.names)) #maxScore
colnames(out.2) <- TF.names
rownames(out.2) <- names(d1)

out.3 <- matrix(0, length(d1), length(TF.names)) #numOcc
colnames(out.3) <- TF.names
rownames(out.3) <- names(d1)

out.4 <- matrix(0, length(d1), length(TF.names)) #sumScore
