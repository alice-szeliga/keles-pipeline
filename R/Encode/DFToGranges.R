dt2granges<-function(dt) { #modified from a function on http://davetang.org/muse/2015/02/04/bed-granges/
  ...
  if(length(df) > 6){
   ...
   if(length(df)==3){
      gr <- with(df, GRanges(chr, IRanges(start+1, end))) #start position increased by 1 bc/ bed files start from 0, and end right after ranges. i.e. the end position is not a peak region.
   } else if (length(df)==4){
...
TargetLabInd<-as.numeric(factor(names(bed.list), levels=unique(names(bed.list))))
